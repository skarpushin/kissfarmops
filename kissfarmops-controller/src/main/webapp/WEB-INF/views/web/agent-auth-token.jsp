...
<head>
	...
	<title><s:message code="app.title" /> - <s:message code="term.agentAuthTokens" /></title>
</head>
<body layout="column">
	<c:import url="/WEB-INF/views/common/page-body-side-nav-begin.jsp" />

	<md-content flex id="content" ng-controller="AgentAuthTokensController" ng-cloak class="ng-cloak">
		<md-card>
			  ...
			  <table md-table ng-model="selected" md-progress="promise">
			    <thead md-head md-order="viewParams.fieldSort" md-on-reorder="reload">
			      <tr md-row>
			        ...
			        <th md-column><span><s:message code="agentAuthToken.token" /></span></th>
			        <th md-column md-order-by="comment" hide-xs hide-sm hide-md show-gt-md><s:message code="term.comment" /></th>
			        <th md-column md-order-by="enabled" hide-xs hide-sm hide-md show-gt-md><s:message code="term.enabled" /></th>
			      </tr>
			    </thead>
			    <tbody md-body>
			      <tr md-row md-auto-select ng-repeat="row in rows" md-select="row" md-select-id="id">
			        ...
			        <td md-cell hide-xs hide-sm hide-md show-gt-md>{{row.comment}}</td>
			        <td md-cell hide-xs hide-sm hide-md show-gt-md>{{row.enabled}}</td>
			      </tr>
			    </tbody>
			  </table>
			</md-table-container>
	...
	<script src="${staticResourcesBase}/static/js/agent-auth-tokens-controller.js"></script>
